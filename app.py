...
request so gunicorn can bind the port and pass Render's health-check
without immediately OOM-crashing on the 512 MB free tier.
"""
...
_model      = None
_device     = None
_enhance_fn = None        # inference.enhance_image
_model_lock = threading.Lock()
_load_error = None        # store any load failure message
...
def _ensure_model_loaded():
    ...
    with _model_lock:
        ...
        try:
            ...
            if ckpt is None:
                raise FileNotFoundError("No checkpoint found in: " + str(paths))
